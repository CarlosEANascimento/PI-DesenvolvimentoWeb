...
if(position[0] - x != 0 || position[1] - y != 0){
	move_and_collide(position[0] - x, position[1] - y, obj_console, 4, 0, 0, 1, 1);
	image_xscale = (sign(position[0] - x) != 0)? sign(position[0] - x) : 1 ;
}
...
if(hired){
	if(instance_exists(obj_enemies)){
		...
		if(distance_to_object(_alvo) < mage.spell_range && image_index == mage.action_frame){
			...
			_spell_inst.spell.origin_x = x + (sign(image_xscale) * mage.spell_source_x);
...
if(!hired && global.on_attack){
	position = [global.target[0], room_height];
}

if(!hired && room_height - y < 16){
	instance_destroy();
}